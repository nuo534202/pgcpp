...
// invoke the comparison function via fmgr. pgcpp simplifies this: the
...
#include "access/nbtpage.hpp"  // BTKeyKind
...
//
// pgcpp additions (so the AM can compare without fmgr):
...
struct ScanKeyData {
    ...
    int sk_attno = 0;
    int sk_strategy = 0;
    pgcpp::catalog::Oid sk_subtype = pgcpp::catalog::kInvalidOid;
    pgcpp::types::Datum sk_argument = 0;
...
// are set from the additional parameters (pgcpp extension so the AM can
