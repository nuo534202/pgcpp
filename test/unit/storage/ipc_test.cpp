...
#include "mytoydb/storage/freespace/fsmpage.hpp"
...
#include "mytoydb/storage/ipc/proc.hpp"
#include "mytoydb/storage/ipc/shm_mq.hpp"
#include "mytoydb/storage/ipc/shmem.hpp"
#include "mytoydb/storage/ipc/shmqueue.hpp"
#include "mytoydb/storage/large_object/inv_api.hpp"
#include "mytoydb/storage/relfilenode.hpp"
#include "mytoydb/storage/sync/sync.hpp"

using mytoydb::storage::AllocateFile;
using mytoydb::storage::AllProcs;
using mytoydb::storage::BytesForFreeSpaceCategory;
using mytoydb::storage::ClearWaitForGraph;
using mytoydb::storage::CloseTransientFiles;
using mytoydb::storage::CreateSharedMemoryAndSemaphores;
using mytoydb::storage::File;
using mytoydb::storage::FileClose;
using mytoydb::storage::FileName;
using mytoydb::storage::FileRead;
using mytoydb::storage::FileSeek;
using mytoydb::storage::FileSync;
using mytoydb::storage::FileTruncate;
using mytoydb::storage::FileWrite;
using mytoydb::storage::FindDeadlockCycle;
using mytoydb::storage::FreeFile;
using mytoydb::storage::FreeSpaceCategoryForBytes;
using mytoydb::storage::FreeSpaceMapDropRel;
using mytoydb::storage::FreeSpaceMapTruncateRel;
using mytoydb::storage::FreeSpaceMapVacuumRel;
using mytoydb::storage::FSMPageClear;
using mytoydb::storage::FSMPageData;
using mytoydb::storage::FSMPageGetFreeSpace;
using mytoydb::storage::FSMPageGetSlot;
using mytoydb::storage::FSMPageInit;
using mytoydb::storage::FSMPageSearchFreeSpace;
using mytoydb::storage::FSMPageSetFreeSpace;
using mytoydb::storage::GetFreeSpaceMapForRel;
using mytoydb::storage::GetLargeObject;
using mytoydb::storage::GetMyProc;
using mytoydb::storage::GetPageWithFreeSpace;
using mytoydb::storage::GetPendingSyncRequests;
using mytoydb::storage::GetPredicateLocks;
using mytoydb::storage::GetSyncStats;
using mytoydb::storage::HasDeadlock;
using mytoydb::storage::HeldLWLockIds;
using mytoydb::storage::InitFileAccess;
using mytoydb::storage::InitializeAllLWLocks;
using mytoydb::storage::InitLatch;
using mytoydb::storage::InitProcess;
using mytoydb::storage::inv_close;
using mytoydb::storage::inv_create;
using mytoydb::storage::inv_drop;
using mytoydb::storage::inv_open;
using mytoydb::storage::inv_read;
using mytoydb::storage::inv_seek;
using mytoydb::storage::inv_tell;
using mytoydb::storage::inv_truncate;
using mytoydb::storage::inv_write;
using mytoydb::storage::kInvalidBlockNumber;
using mytoydb::storage::kInvalidFile;
using mytoydb::storage::kInvalidLargeObjectOid;
using mytoydb::storage::kInvRdwr;
using mytoydb::storage::kInvWrite;
using mytoydb::storage::kOAppend;
using mytoydb::storage::kOCreate;
using mytoydb::storage::kOExclusive;
using mytoydb::storage::kOReadOnly;
using mytoydb::storage::kOReadWrite;
using mytoydb::storage::kWaitLatchSet;
using mytoydb::storage::kWaitTimeout;
using mytoydb::storage::LargeObject;
using mytoydb::storage::LargeObjectDesc;
using mytoydb::storage::LargeObjectOid;
using mytoydb::storage::Latch;
using mytoydb::storage::LookupNamedLock;
using mytoydb::storage::LWLock;
using mytoydb::storage::LWLockAcquire;
using mytoydb::storage::LWLockConditionalAcquire;
using mytoydb::storage::LWLockHeldByMe;
using mytoydb::storage::LWLockHeldByMeInMode;
using mytoydb::storage::LWLockId;
using mytoydb::storage::LWLockInitialize;
using mytoydb::storage::LWLockMode;
using mytoydb::storage::LWLockRelease;
using mytoydb::storage::NumHeldLWLocks;
using mytoydb::storage::NumLargeObjects;
using mytoydb::storage::NumOpenFiles;
using mytoydb::storage::NumPendingSyncRequests;
using mytoydb::storage::NumPredicateLocks;
using mytoydb::storage::NumProcs;
using mytoydb::storage::NumShmemRegions;
using mytoydb::storage::PathNameOpenFile;
using mytoydb::storage::PGPROC;
using mytoydb::storage::PredicateLockConflicts;
using mytoydb::storage::PredicateLockPage;
using mytoydb::storage::PredicateLockRelation;
using mytoydb::storage::PredicateLockRelease;
using mytoydb::storage::PredicateLockReleaseAll;
using mytoydb::storage::PredicateLockTuple;
using mytoydb::storage::ProcGlobalReset;
using mytoydb::storage::RecordPageWithFreeSpace;
using mytoydb::storage::RegisteredIPCInitFns;
using mytoydb::storage::RegisterIPCInitFn;
using mytoydb::storage::RegisterSyncRequest;
using mytoydb::storage::RegisterWaitFor;
using mytoydb::storage::RelFileNode;
using mytoydb::storage::ResetAllLWLocks;
using mytoydb::storage::ResetFreeSpaceMap;
using mytoydb::storage::ResetIPCInitFns;
using mytoydb::storage::ResetLargeObjects;
using mytoydb::storage::ResetLatch;
using mytoydb::storage::ResetShmem;
using mytoydb::storage::ResetSyncQueue;
using mytoydb::storage::ResetVfdCache;
using mytoydb::storage::SetLatch;
using mytoydb::storage::SetMyProc;
using mytoydb::storage::ShmemInitStruct;
using mytoydb::storage::ShmemSize;
using mytoydb::storage::ShmMq;
using mytoydb::storage::ShmMqCreate;
using mytoydb::storage::ShmMqDestroy;
using mytoydb::storage::ShmMqResult;
using mytoydb::storage::SHMQueue;
using mytoydb::storage::SHMQueueDelete;
using mytoydb::storage::SHMQueueElem;
using mytoydb::storage::SHMQueueEmpty;
using mytoydb::storage::SHMQueueInit;
using mytoydb::storage::SHMQueueLength;
using mytoydb::storage::SHMQueuePop;
using mytoydb::storage::SHMQueuePush;
using mytoydb::storage::SHMQueuePushTail;
using mytoydb::storage::SyncFile;
using mytoydb::storage::SyncRequest;
using mytoydb::storage::SyncRequestTag;
using mytoydb::storage::SyncStats;
using mytoydb::storage::TestLatch;
using mytoydb::storage::UnregisterWaitFor;
using mytoydb::storage::WaitEventSet;
using mytoydb::storage::WaitForEdges;
using mytoydb::storage::WaitLatch;
...
TEST_F(IpcTest, WaitEventSetAcceptsLatchAndSocketEvents) {
    ...
    InitLatch(&latch);
    set.AddLatch(&latch);
    set.AddSocket(/*fd=*/-1, mytoydb::storage::kWaitSocketReadable);
...
TEST_F(IpcTest, WaitEventSetWaitsForLatch) {
    ...
    set.AddLatch(&latch);
    SetLatch(&latch);
    std::vector<mytoydb::storage::WaitEvent> occurred;
    int n = mytoydb::storage::WaitEventSetWait(&set, /*timeout_ms=*/100, &occurred);
    EXPECT_GE(n, 1);
...
TEST_F(IpcTest, ProcessSyncRequestsClearsQueue) {
    ...
    int processed = mytoydb::storage::ProcessSyncRequests();
...
TEST_F(IpcTest, SyncStatsTrackCounts) {
    ...
    RegisterSyncRequest(SyncRequestTag{rnode, 0, 1});
    RegisterSyncRequest(SyncRequestTag{rnode, 0, 2});
    mytoydb::storage::ProcessSyncRequests();
...
TEST_F(IpcTest, FreeSpaceCategoryForBytesClampsToMax) {
    EXPECT_EQ(FreeSpaceCategoryForBytes(100000), mytoydb::storage::kFsmFreeSpaceMax);
}
