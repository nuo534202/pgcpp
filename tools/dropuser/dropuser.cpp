// dropuser.cpp — pgcpp DROP ROLE utility (dropuser equivalent).
//
// Converted from PostgreSQL 15's src/bin/dropuser/.
//
// Connects to a running server and issues a DROP ROLE statement.
//
// Usage:
//   dropuser [-h host] [-p port] [-d connect_db] [--if-exists] <name>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <string>
...
void PrintUsage(const char* prog_name) {
    std::fprintf(stderr, "pgcpp drop-user utility (dropuser equivalent)\n\n");
...
int main(int argc, char* argv[]) {
    ...
    AdminResult r = DropRole(host, port, connect_db, name, if_exists);
    if (r != AdminResult::kOk) {
        std::fprintf(stderr, "dropuser: failed (code %d)\n", static_cast<int>(r));
        return 1;
    }
    std::fprintf(stdout, "DROP ROLE\n");
    return 0;
}
