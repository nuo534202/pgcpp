# Code style targets (clang-format / clang-tidy) for MyToyDB
#
...
function(mytoydb_setup_codestyle_targets)
    find_program(CLANG_FORMAT_BIN clang-format)
    find_program(CLANG_TIDY_BIN clang-tidy)

    file(GLOB_RECURSE MYTOYDB_FORMAT_FILES
    ...
    file(GLOB_RECURSE MYTOYDB_TIDY_FILES
        "${CMAKE_SOURCE_DIR}/src/*.cpp"
    )

    list(JOIN MYTOYDB_FORMAT_FILES " " MYTOYDB_FORMAT_FILES_STR)
    list(JOIN MYTOYDB_TIDY_FILES " " MYTOYDB_TIDY_FILES_STR)

    if(CLANG_FORMAT_BIN)
        if(MYTOYDB_FORMAT_FILES)
            add_custom_target(format
                COMMAND ${CLANG_FORMAT_BIN} -i ${MYTOYDB_FORMAT_FILES}
            ...
            add_custom_target(format-check
                COMMAND ${CLANG_FORMAT_BIN} --dry-run --Werror ${MYTOYDB_FORMAT_FILES}
                WORKING_DIRECTORY ${CMAKE_SOURCE_DIR}
    ...
    if(CLANG_TIDY_BIN)
        if(MYTOYDB_TIDY_FILES)
            add_custom_target(tidy
                COMMAND ${CLANG_TIDY_BIN} ${MYTOYDB_TIDY_FILES}
                WORKING_DIRECTORY ${CMAKE_SOURCE_DIR}
