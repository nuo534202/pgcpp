// parser.cpp — Raw parser entry point for pgcpp.
//
...
// safely destructed.
void pgcpp_parser::BisonParser::error(const location_type& loc, const std::string& msg) {
...
std::vector<RawStmt*> raw_parser(const std::string& str) {
    ...
    int result = 0;
    {
        ...
        pgcpp_parser::BisonParser parser(driver);
        if (::getenv("YYDEBUG"))
    ...
    if (result != 0) {
        ...
        ereport(pgcpp::error::LogLevel::kError, "syntax error in input SQL");
    }
