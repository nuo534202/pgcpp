...
//
// MyToyDB simplification: the routine table is typed to the btree scan
...
constexpr mytoydb::catalog::Oid kBTreeAmOid = 403;    // BTREE_AM_OID
constexpr mytoydb::catalog::Oid kHashAmOid = 405;     // HASH_AM_OID
constexpr mytoydb::catalog::Oid kGistAmOid = 783;     // GIST_AM_OID
constexpr mytoydb::catalog::Oid kGinAmOid = 2742;     // GIN_AM_OID
constexpr mytoydb::catalog::Oid kBrinAmOid = 3580;    // BRIN_AM_OID
constexpr mytoydb::catalog::Oid kSpgistAmOid = 4000;  // SPGIST_AM_OID
...
struct IndexAmRoutine {
    mytoydb::catalog::Oid amoid = mytoydb::catalog::kInvalidOid;
    ...
    bool (*aminsert)(Relation index, BTKeyKind kind, const void* key, uint16_t key_len,
                     const mytoydb::transaction::ItemPointerData& tid) = nullptr;
    ...
    int64_t (*amgetbitmap)(BTScanDesc scan,
                           std::vector<mytoydb::transaction::ItemPointerData>* tids) = nullptr;
};
...
// unknown. MyToyDB only implements btree (kBTreeAmOid = 403).
const IndexAmRoutine* LookupAmRoutine(mytoydb::catalog::Oid amoid);

// --- Generic index entry points (dispatch to the AM routine) ---

// index_open — open an index relation by OID (PG alias for RelationOpen).
Relation index_open(mytoydb::catalog::Oid relid);
...
int64_t index_getbitmap(BTScanDesc scan, std::vector<mytoydb::transaction::ItemPointerData>* tids);
