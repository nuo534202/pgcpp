...
Buffer _bt_create_page(Relation index, BlockNumber block_num,
                       uint32_t flags, uint32_t level) {
    char pagebuf[kBlckSz];
    _bt_init_page(pagebuf, flags, level);
    index->rd_smgr = RelationGetSmgr(index);
    mytoydb::storage::smgrextend(index->rd_smgr, ForkNumber::kMain,
                                 block_num, pagebuf, false);
    return ReadBuffer(index->rd_smgr, ForkNumber::kMain, block_num,
...
// if the page is full.
OffsetNumber _bt_page_insert_item(Page page, const void* item_data,
                                   uint16_t item_size,
    ...
    auto* new_lp = reinterpret_cast<ItemIdData*>(
        page + kPageHeaderSize) + (insert_offset - 1);
...
std::vector<LeafEntry> _bt_read_all_items(Page page) {
    ...
    for (OffsetNumber off = 1; off <= max_offset; off++) {
        ...
        if (!ItemIdIsNormal(item_id)) continue;
...
OffsetNumber _bt_find_insert_pos(Page page, BTKeyKind kind,
...
OffsetNumber _bt_find_scan_pos(Page page, BTKeyKind kind,
                                const void* key, uint16_t key_len) {
    OffsetNumber max_offset = PageGetMaxOffsetNumber(page);
    for (OffsetNumber off = 1; off <= max_offset; off++) {
        auto* item_id = PageGetItemId(page, off);
...
void btbuild(Relation index, BTKeyKind key_kind) {
    ...
    mytoydb::storage::smgrextend(index->rd_smgr, ForkNumber::kMain, 0,
                                 metabuf, false);

    // Block 1: root leaf page.
    char rootbuf[kBlckSz];
    _bt_init_page(rootbuf, kBtpLeaf | kBtpRoot, 0);
    mytoydb::storage::smgrextend(index->rd_smgr, ForkNumber::kMain, 1,
                                 rootbuf, false);
}
...
bool btinsert(Relation index, BTKeyKind kind, const void* key,
              uint16_t key_len, const ItemPointerData& tid) {
    index->rd_smgr = RelationGetSmgr(index);

    // Read meta page to find root.
    Buffer meta_buf = ReadBuffer(index->rd_smgr, ForkNumber::kMain, 0,
                                 ReadBufferMode::kNormal);
    ...
    Buffer root_buf = ReadBuffer(index->rd_smgr, ForkNumber::kMain,
                                 root_block, ReadBufferMode::kNormal);
    ...
    if (_bt_is_leaf_page(root_page)) {
        ...
        OffsetNumber insert_pos = _bt_find_insert_pos(root_page, kind,
                                                      key, key_len);
        OffsetNumber result = _bt_page_insert_item(root_page, item_buf,
                                                    item_size, insert_pos);
        ...
        for (size_t i = 0; i < entries.size(); i++) {
            ...
            int cmp = _bt_compare_keys(kind, key, key_len,
                                       item_key, item_key_len);
            if (cmp <= 0) {
                insert_idx = i;
                break;
            }
        ...
        Buffer new_leaf_buf = _bt_create_page(index, new_leaf_block,
                                              kBtpLeaf, 0);
        ...
        for (size_t i = 0; i < mid; i++) {
            PageAddItem(root_page, reinterpret_cast<Item>(entries[i].data.data()),
                        entries[i].size, kInvalidOffsetNumber, false);
        }

        // Write right half to new leaf.
        for (size_t i = mid; i < entries.size(); i++) {
            PageAddItem(new_leaf_page,
                        reinterpret_cast<Item>(entries[i].data.data()),
                        entries[i].size, kInvalidOffsetNumber, false);
        }
        ...
        Buffer new_root_buf = _bt_create_page(index, new_root_block,
                                              kBtpRoot, 1);
        ...
        {
            struct { BlockNumber child; } entry;
            entry.child = root_block;
            PageAddItem(new_root_page, reinterpret_cast<Item>(&entry),
                        sizeof(entry), kInvalidOffsetNumber, false);
        }

        // Second entry: child = new leaf, key = first key in new leaf.
        {
            auto* first_item = reinterpret_cast<BTItem>(
                entries[mid].data.data());
            ...
            std::memcpy(int_entry.data() + sizeof(BlockNumber),
                        first_key, first_key_len);
            PageAddItem(new_root_page, reinterpret_cast<Item>(int_entry.data()),
                        int_entry_size, kInvalidOffsetNumber, false);
        }
        ...
        Buffer meta_buf2 = ReadBuffer(index->rd_smgr, ForkNumber::kMain, 0,
                                      ReadBufferMode::kNormal);
    ...
    for (OffsetNumber off = 1; off <= max_offset; off++) {
        ...
        if (data_len < sizeof(BlockNumber)) continue;
    ...
    Buffer leaf_buf = ReadBuffer(index->rd_smgr, ForkNumber::kMain,
                                 child_block, ReadBufferMode::kNormal);
    Page leaf_page = BufferGetPage(leaf_buf);

    OffsetNumber insert_pos = _bt_find_insert_pos(leaf_page, kind,
                                                   key, key_len);
    OffsetNumber result = _bt_page_insert_item(leaf_page, item_buf,
                                                item_size, insert_pos);
    ...
    for (OffsetNumber off = 2; off <= root_max; off++) {
        ...
        const void* entry_key = data + sizeof(BlockNumber);
        uint16_t entry_key_len = data_len - sizeof(BlockNumber);
        int cmp = _bt_compare_keys(kind, first_key, first_key_len,
                                   entry_key, entry_key_len);
        if (cmp <= 0) {
            root_insert_pos = off;
            break;
    ...
    OffsetNumber root_result = _bt_page_insert_item(root_page,
                                                     int_entry.data(),
                                                     int_entry_size,
                                                     root_insert_pos);
...
Buffer _bt_search_leaf(Relation index, BTKeyKind kind,
                       const void* key, uint16_t key_len,
                       OffsetNumber* offset) {
    index->rd_smgr = RelationGetSmgr(index);
    ...
    // Read the leaf page.
    Buffer leaf_buf = ReadBuffer(index->rd_smgr, ForkNumber::kMain,
                                 leaf_block, ReadBufferMode::kNormal);
    Page leaf_page = BufferGetPage(leaf_buf);
...
static bool _bt_entry_matches(BTKeyKind kind, const void* entry_key,
                              uint16_t entry_key_len,
                              const BTScanKeyData& scan_key) {
    if (scan_key.strategy == BTStrategy::kAll || scan_key.key == nullptr) {
        return true;
    }

    int cmp = _bt_compare_keys(kind, entry_key, entry_key_len,
                               scan_key.key, scan_key.key_len);
...
bool btgettuple(BTScanDesc scan) {
    ...
    if (!scan->inited) {
        ...
        if (start_from_beginning) {
            ...
            Buffer meta_buf = ReadBuffer(index->rd_smgr, ForkNumber::kMain,
                                         0, ReadBufferMode::kNormal);
            Page meta_page = BufferGetPage(meta_buf);
            BTMetaPageData meta = _bt_get_meta(meta_page);
            ReleaseBuffer(meta_buf);

            scan->curr_block = meta.root;
            scan->curr_buf = ReadBuffer(index->rd_smgr, ForkNumber::kMain,
                                        scan->curr_block,
                                        ReadBufferMode::kNormal);
            ...
            if (!_bt_is_leaf_page(page)) {
                ...
                if (max_off >= 1) {
                    ...
                    scan->curr_buf = ReadBuffer(index->rd_smgr,
                                                ForkNumber::kMain,
                                                scan->curr_block,
                                                ReadBufferMode::kNormal);
                }
            ...
            scan->curr_buf = _bt_search_leaf(index, scan->key_kind,
                                             scan->scan_key.key,
                                             scan->scan_key.key_len,
                                             &start_offset);
            Page page = BufferGetPage(scan->curr_buf);
    ...
    while (true) {
        ...
        while (scan->curr_offset <= max_offset) {
            ...
            auto* bt_item = reinterpret_cast<BTItem>(
                PageGetItem(page, item_id));
            const void* entry_key = _bt_item_get_key(bt_item);
            uint16_t entry_key_len = _bt_item_get_key_len(
                ItemIdGetLength(item_id));

            if (_bt_entry_matches(scan->key_kind, entry_key, entry_key_len,
                                  scan->scan_key)) {
            ...
            if (scan->scan_key.key != nullptr) {
                ...
                bool has_upper_bound =
                    strat == BTStrategy::kEqual ||
                    strat == BTStrategy::kLess ||
                    strat == BTStrategy::kLessEqual;
                if (has_upper_bound) {
                    int cmp = _bt_compare_keys(scan->key_kind,
                                               entry_key, entry_key_len,
                                               scan->scan_key.key,
                                               scan->scan_key.key_len);
                    // kEqual/kLessEqual: stop past the key (cmp > 0).
                    // kLess: stop at or past the key (cmp >= 0).
                    bool stop = (strat == BTStrategy::kLess)
                                    ? (cmp >= 0)
                                    : (cmp > 0);
                    if (stop) {
                        ReleaseBuffer(scan->curr_buf);
