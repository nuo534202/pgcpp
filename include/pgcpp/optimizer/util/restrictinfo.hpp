...
// tracking, merge/hash join eligibility, etc. For pgcpp's single-table
...
struct RestrictInfo {
    pgcpp::parser::Node* clause = nullptr;     // the wrapped qual expression
    ...
    pgcpp::catalog::Oid hashjoinoperator = 0;  // hash-join operator OID
    ...
    pgcpp::catalog::Oid opno = 0;  // operator OID (for op family lookup)
    ...
    pgcpp::catalog::Oid mergeopfamilies = 0;
};

// make_restrictinfo — create a RestrictInfo wrapping a clause.
//
// Mirrors PG's make_restrictinfo. For pgcpp, required_relids and
// incompatible_relids are simplified to vectors of rel indexes.
RestrictInfo* make_restrictinfo(PlannerInfo* root, pgcpp::parser::Node* clause, bool can_join,
                                bool pseudoconstant, Relids required_relids,
                                Relids incompatible_relids, pgcpp::catalog::Oid hashjoinoperator);
...
std::vector<RestrictInfo*> make_restrictinfos_from_quals(
    PlannerInfo* root, const std::vector<pgcpp::parser::Node*>& clauses);
